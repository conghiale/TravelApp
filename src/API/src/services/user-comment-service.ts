...
class UserCommentService {
    getCommentsByDestinationId = async (destinationId: mongoose.Types.ObjectId) => {
        const cmts = await UserComment.find({
            destinationId: destinationId,
            isDeleted: false,
        }, { __v: 0 }).sort({ updatedAt: -1 });

        const data = await Promise.all(cmts.map(async cmt => {
            ...
            return {
                ...
                createdAt: daytimeCreated.getDate() + '-' + 
                            daytimeCreated.getMonth() + 1 + '-' + 
                            daytimeCreated.getFullYear() + ' | ' +
                            daytimeCreated.getHours() + ':' +
                            daytimeCreated.getMinutes() + ':' +
                            daytimeCreated.getSeconds(),
                updatedAt: daytimeUpdate.getDate() + '-' + 
                            daytimeUpdate.getMonth() + 1 + '-' + 
