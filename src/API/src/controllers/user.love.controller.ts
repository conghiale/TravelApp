...
class UserLoveController {
  ...
  getLoveListByUserId = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.userLoveService.getLoveListByUserId(parseObjectId(id), (order === orderConstant.DESCENDING ? true : false));
      if(result.success) {
        return res.send({message: result.message, data: result.data});
      }
      return res.status(400).send({message: result.message});
    } catch (error) {
      return res.status(500).send({message: 'Internal error in getLoveListByUserId'});
    }
  };
  ...
  toggleLoveItem = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).send({message: 'Internal error in toggleLoveItem'})
    }
  }
  ...
  addLoveItem = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.userLoveService.addLoveItem(parseObjectId(userId), parseObjectId(destId))
  ...
  removeLoveItem = async(req: Request, res: Response) => {
    try {
      ...
      const result = await this.userLoveService.removeLoveItem(parseObjectId(userId), parseObjectId(destId));
      return res.status(result.success ? 200 : 400).send({message: result.message});
    } catch (error) {
      return res.status(500).send({message: 'Internall error in removeLoveItem'})
    }
  };
}
